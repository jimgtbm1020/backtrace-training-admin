...
export default function VersionHistoryPage(){
  ...
  return <main className="shell"><header className="header"><div><div className="brand">Version History</div><div className="subtitle">Read-only application release history</div></div><a href="/">Back to dashboard</a></header><section className="card"><button disabled={loading} onClick={load}>Refresh history</button><button disabled={!versions.length} onClick={downloadCsv}>Download CSV</button>{error&&<p>{error}</p>}{versions.map(version=><article key={version.id}><h2>{version.version||'—'} {version.is_current&&<span className="pill">Current</span>}</h2><p>{version.title||'—'} · {version.release_date||'—'}</p><p>{version.summary||'—'}</p>{changes.filter(change=>change.version_id===version.id).map(change=><p key={change.id}>• {change.component||'Change'}: {change.change_summary||'—'}</p>)}</article>)}</section></main>;
}