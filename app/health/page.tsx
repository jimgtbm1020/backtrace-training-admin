...
export default function HealthPage(){
  ...
  async function run(){setLoading(true);setError('');setResults({});const {data:auth}=await supabase.auth.getUser();if(!auth.user){setError('Administrator sign-in is required.');setLoading(false);return;}const {data:profile,error:profileError}=await supabase.from('profiles').select('role,active').eq('id',auth.user.id).maybeSingle();if(profileError||profile?.role!=='admin'||!profile.active){setError(profileError?.message||'Administrator access is required.');setLoading(false);return;}const next:Record<string,string>={authentication:'Passed'};for(const [table] of checks){const {error}=await supabase.from(table).select('*',{count:'exact',head:true});next[table]=error?'Failed: '+error.message:'Passed';}setResults(next);setLoading(false);}
