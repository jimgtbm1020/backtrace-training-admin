...
export default function ClassesPage(){
  ...
  function downloadCsv(){const header=['Session','Request','State','Registration','Check-in open','Check-in close'];const lines=filtered.map(row=>[row.id,row.request_id,row.active?'Active':'Closed',row.registration_open?'Open':'Closed',row.checkin_open_at,row.checkin_close_at].map(value=>'"'+String(value??'').replaceAll('"','""')+'"').join(','));const blob=new Blob([[header.join(','),...lines].join('\\n')],{type:'text/csv;charset=utf-8'});const url=URL.createObjectURL(blob);const link=document.createElement('a');link.href=url;link.download='backtrace-classes-attendance.csv';link.click();URL.revokeObjectURL(url);}
